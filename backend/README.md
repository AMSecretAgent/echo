...
> **New: Echo is now an AI Venture Agent.** The demand→product pipeline below
> now runs as a real multi-agent LangGraph system — six specialist agents
> (Demand Discovery → Market Validation → Product Creation → Revenue → Launch
> → Execution) coordinated by a supervisor that retries low-confidence steps
> and streams live status to the dashboard. See **"Run the Agentic layer"**
> further down. Everything described below it — receipts, trackable links,
> the UPI buyer page, click tracking, Instagram/screenshot/file ingestion,
> disclosed clone replies — is unchanged; the agents call it as tools.
...
This is what actually powers the dashboard now. It uses the same `.env` as above.

```bash
cd backend
pip install -r requirements.txt      # adds langgraph + langchain-core
uvicorn main:app --reload --port 8000
```
...
Open http://localhost:5173, paste some comments, and hit **Listen to my
audience**. You'll see all six agents (Demand Discovery, Market Validation,
Product Creation, Revenue, Launch, Execution) run in sequence, streaming
live over SSE from `/api/opportunities/{run_id}/stream`, ending in an
Opportunity Report with a real trackable link and UPI checkout page — no
API key required (falls back to a local heuristic engine), and upgrades
automatically to real LLM output the moment `LLM_API_KEY` is set.

**Note on the reconstructed pieces:** `backend/routes/links.py` and the
`Product`/`TrackedLink`/`LinkClick` models in `backend/db/models.py` are
*rebuilt* from what your README describes (trackable `/r/{id}` links, the
`/p/{id}` UPI buyer page, `/api/stats/{id}`) — I didn't have your real
`main.py` or clustering engine to diff against. If you already have working
versions of these, keep yours and delete `routes/links.py` here so clicks
aren't double-counted; the agents only need `link_tool.py` and
`checkout_tool.py` to point at your real implementations.
